...
const Packet = struct {
    ...
    fn deinit(self: *Packet, allocator: *std.mem.Allocator) void {
        if (self.type == 4) {
            self.payload.literal.deinit(allocator);
        } else {
            // TODO fix deinit
...
const Buffer = struct {
    ...
    fn readPacket(self: *Buffer, allocator: *std.mem.Allocator) !Packet {
        ...
        const payload = blk: {
            if (t == 4) {
                const lit = try self.readLiteral(allocator);
    ...
    fn readLiteral(self: *Buffer, allocator: *std.mem.Allocator) !List(u4) {
        var lit = List(u4){};

        var chunk: u5 = self.readInt(u5);
        try lit.append(allocator, @truncate(u4, chunk));
