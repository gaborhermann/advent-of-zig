...
pub fn find_last_winner(boards: anytype, index: anytype, numbers: anytype, allocator: anytype) !u32 {
    const num_boards: u32 = @truncate(u32, boards.items.len);
    var winners: u32 = 0;

    var won = try allocator.alloc(bool, num_boards);
    defer allocator.free(won[0..num_boards]);
        ...
        if (index.get(num)) |positions| {
            for (positions.items) |pos| {
                ...
                if (b.row_marked[pos.row] == 5 or b.col_marked[pos.col] == 5) {
                    ...
                    if (winners == num_boards) {
                        // we've got the last winner
...
pub fn main() !void {
    ...
    var score = find_last_winner(boards, index, numbers, allocator);
